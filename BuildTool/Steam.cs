...
namespace BuildTool {
	public static class Steam {
		...
		private static SteamGameData SearchAllInstallations(
			...
			while(true) {
				steamLibraryIndex++;
				if(!steamLibraryData.TryGetValue(steamLibraryIndex.ToString(), out var steamLibraryPath))
					return null;
		...
		private static SteamGameData GameDataFromAppManifest(string manifestFile) {
			...
			if(!gameData.TryGetValue("name", out var gameName))
				return null;
			if(!gameData.TryGetValue("appid", out var appidStr))
				return null;
			if(!uint.TryParse(appidStr, out var appId))
				return null;
			if(!gameData.TryGetValue("installdir", out var gameInstallFolderName))
				return null;
			var gameDir =
